...
class CheckDBSupportsConstraints(Task):
    def __init__(self):
    ...
    def install(self, environment):
        ...
        try:
            ...
            conn = None
        supported = True
        if dbn.startswith('mysql'):
          res       = conn.execute("show variables like 'have_innodb'")
          supported = (res.fetchone()[1] == "YES")
        if not supported:
            return Task.failure
        return Task.success
...
if __name__ == '__main__':
    ...
    class CheckDBSupportsConstraintsTest(unittest.TestCase):
        def runTest(self):
            # Read db config.
            cfg = RawConfigParser()
            ...
            task        = CheckDBSupportsConstraints()
    ...
    testcase = CheckDBSupportsConstraintsTest()
